...
public class JsonArrayContainsFunction extends ScalarFunction {
	...
	public Boolean eval(Object... values) {
		...
		JSONArray array;
		if (values[ARRAY_IDX] instanceof String) {
			String data = (String) values[ARRAY_IDX];
			array = JSON.parseArray(data);
		} else if (values[ARRAY_IDX] instanceof JSONArray) {
			array = (JSONArray) values[ARRAY_IDX];
		} else {
			throw new RuntimeException("unknown type " + values[ARRAY_IDX]);
		}
