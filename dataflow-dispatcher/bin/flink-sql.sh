...
for jar in `find "$FLINK_OPT_DIR/connectors" -name *.jar`; do
    if [[ -z $FLINK_CONNECTORS_JARS_ARGS ]]; then
...
exec $JAVA_RUN $JVM_ARGS "${log_setting[@]}" -classpath "`manglePathList "$CC_CLASSPATH:$INTERNAL_HADOOP_CLASSPATHS:$FLINK_SQL_CLIENT_JARS"`" io.infinivision.flink.core.DispatcherConsoleServer "$@" $FLINK_CONNECTORS_JARS_ARGS
